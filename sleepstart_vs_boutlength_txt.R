sleepstart_vs_boutlength_txt <- function(analysR_output, xlsx_file_name, channels = 1:32){
    ...
    # Add 1's or 0's as a third column to each dataframe in sleepstart_vs_boutlength_dfs.
    # This way, you know which sleep episode (and corresponding length) happened during
    ...
    for(df in sleepstart_vs_boutlength_dfs){
        ...
        for(bout in df$sleep_start){
            ...
            identifier <- light_regime[bout]
            
            light_regime_current[j] <- identifier
            
            j <- j + 1
        }
        
        sleepstart_vs_boutlength_dfs[[i]] <- cbind(sleepstart_vs_boutlength_dfs[[i]], light_regime = light_regime_current)
